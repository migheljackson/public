...
if ( isset( $cached_str ) && !isset( $_REQUEST["reset_cache"] ) ) {
  ...
  if ( $popular_item_response->status == 200 ) {
    ...
    if ( !empty( $pop_items->categories ) ) {
      $render = true;
      $category_list = '<ul id="topics" class="show-for-medium-up small-12 large-12 columns">';
    ...
    $activity_list = '<ul id="activities" class="show-for-medium-up medium-12 large-12 columns">';
